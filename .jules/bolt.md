...
## 2026-03-30 - File Descriptor Table Allocation Hint Optimization
**Learning:** In `kernel/src/process/fd.rs` and `kernel/src/process/task.rs`, file descriptor allocations (`open`, `dup`, `pipe`, `socket`, `fcntl`) were scanning `fd_table.entries` from index 0 on every call, leading to O(N) linear search overhead as the number of open descriptors grew. Adding a `next_free_fd: usize` hint to `FdTable` turns typical descriptor allocation scans into O(1) fast-path lookups while maintaining POSIX semantics on `close` via `fd_table.next_free_fd.min(closed_fd)`.
**Action:** For table/array-backed resources that require returning the lowest free index, maintain an explicit lowest-free-index hint to avoid repeated O(N) scans from index 0.
