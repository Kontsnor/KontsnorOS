...
## 2026-03-30 - Redundant Syscall Dispatcher Query Removal & KRwLock Spin Ordering Optimization
**Learning:** In `kernel/src/syscall/mod.rs`, `syscall_dispatch_rust` was executing an unneeded `current_pid()` call on every non-fastpath system call dispatch and discarding the result (`let _ = current_pid_val;`), causing unnecessary LAPIC/GS scratch queries and scheduler spinlock attempts on every syscall entry. Additionally, in `kernel/src/sync/rwlock.rs`, using `Ordering::Acquire` while spin-checking `writer_pending` generated excessive memory fences during polling loops; switching to `Ordering::Relaxed` eliminates pipeline stalls.
**Action:** Audit syscall dispatch paths for leftover diagnostic calls and ensure spin-loop flag checks use `Ordering::Relaxed` on x86_64.
