...
#[test_case]
fn test_ext4_64bit_descriptors_and_extent_merging() {
    kprintln!("[test] Starting Ext4 64-bit descriptors and extent merging test...");

    // Test GroupDescriptor 64-bit address calculations
    let mut gd = crate::fs::ext::types::GroupDescriptor {
        bg_block_bitmap: 0x1000,
        bg_inode_bitmap: 0x2000,
        bg_inode_table: 0x3000,
        bg_free_blocks_count: 500,
        bg_free_inodes_count: 100,
        bg_used_dirs_count: 5,
        bg_pad: 0,
        bg_reserved: [0; 12],
        bg_block_bitmap_hi: 0x0001,
        bg_inode_bitmap_hi: 0x0002,
        bg_inode_table_hi: 0x0003,
    ...
    assert_eq!(gd.block_bitmap(false), 0x1000);
    assert_eq!(gd.block_bitmap(true), 0x1_0000_1000);

    assert_eq!(gd.inode_bitmap(false), 0x2000);
    assert_eq!(gd.inode_bitmap(true), 0x2_0000_2000);

    assert_eq!(gd.inode_table(false), 0x3000);
    assert_eq!(gd.inode_table(true), 0x3_0000_3000);

    assert_eq!(gd.free_blocks_count(false), 500);
    assert_eq!(gd.free_blocks_count(true), (2 << 16) | 500);

    gd.set_free_blocks_count(true, 0x0003_0020);
    assert_eq!(gd.free_blocks_count(true), 0x0003_0020);

    // Test 64-bit raw inode size helper
    let mut raw_inode = crate::fs::ext::types::ExtRawInode::zeroed();
    raw_inode.i_mode = 0x81a4; // Regular file
    raw_inode.set_file_size(0x0000_000A_8000_0000);

    assert_eq!(raw_inode.file_size(), 0x0000_000A_8000_0000);
    let size_lo = raw_inode.i_size;
    let size_hi = raw_inode.i_dir_acl;
    assert_eq!(size_lo, 0x8000_0000);
    assert_eq!(size_hi, 10);

    // Test extent merging via ExtInode::insert_extent
    let device = crate::drivers::ramdisk::create_ext2_ramdisk();
    let fs = crate::fs::ext::ExtFileSystem::mount(device.clone()).expect("Failed to mount ext");
    let inode = fs.get_ext_inode(12).expect("Failed to get ext inode");

    let mut ext_raw = crate::fs::ext::types::ExtRawInode::zeroed();
    ext_raw.i_mode = 0x81a4;

    // Insert block 0 -> phys block 100
    inode.insert_extent(&mut ext_raw, 0, 100, 1).expect("Insert extent 0 failed");
    // Insert block 1 -> phys block 101 (should merge into block 0..2)
    inode.insert_extent(&mut ext_raw, 1, 101, 1).expect("Insert extent 1 failed");

    // Verify extent tree merged into 1 extent with len = 2
    let i_block = ext_raw.i_block;
    assert_eq!(inode.resolve_extent_block(&i_block, 0).unwrap(), 100);
    assert_eq!(inode.resolve_extent_block(&i_block, 1).unwrap(), 101);

    let header = unsafe {
        core::ptr::read_unaligned(i_block.as_ptr() as *const crate::fs::ext::types::Ext4ExtentHeader)
    };
    let entries = header.eh_entries;
    assert_eq!(entries, 1);

    kprintln!("[test] Ext4 64-bit descriptors and extent merging test PASSED!");
}
