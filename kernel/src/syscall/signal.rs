...
pub fn sys_rt_sigaction(
    ...
    if signum == 9 || signum == 19 { // SIGKILL, SIGSTOP cannot be caught
        return Errno::EINVAL.into();
    }
    
    if !act.is_null() && !crate::syscall::fs::validate_user_ptr(act as *const u8, core::mem::size_of::<crate::process::task::SigAction>()) {
        return Errno::EFAULT.into();
    }
    if !oldact.is_null() && !crate::syscall::fs::validate_user_ptr(oldact as *const u8, core::mem::size_of::<crate::process::task::SigAction>()) {
        return Errno::EFAULT.into();
    }
...
pub fn sys_rt_sigprocmask(
    ...
    if !set.is_null() && !crate::syscall::fs::validate_user_ptr(set as *const u8, core::mem::size_of::<u64>()) {
        return Errno::EFAULT.into();
    }
    if !oldset.is_null() && !crate::syscall::fs::validate_user_ptr(oldset as *const u8, core::mem::size_of::<u64>()) {
        return Errno::EFAULT.into();
    }
...
pub fn handle_pending_signals(regs: *mut super::SavedRegisters) {
    ...
    if action.sa_handler == 1 { // SIG_IGN
        return;
    } else if action.sa_handler == 0 { // SIG_DFL
        if sig == 17 || sig == 18 || sig == 28 {
            return;
        }
        kprintln!("[signal] Default action for signal {} is termination. Exiting task.", sig);
        ...
        if !crate::syscall::fs::validate_user_ptr(new_user_sp as *const u8, core::mem::size_of::<SignalFrame>()) {
        ...
        kprintln!("[signal] Delivered signal {} to custom handler at {:#x}, trampoline user stack: {:#x}", sig, action.sa_handler, new_user_sp);
    }
}
...
pub fn sys_rt_sigreturn(regs: *mut super::SavedRegisters) -> SyscallResult {
    let user_sp = unsafe { (*regs).rsp };
    // The user stack pointer currently points to user_sp + 8 because the `ret`
    // instruction popped the return address (sa_restorer). Thus, the SignalFrame
    // is located exactly at user_sp - 8.
    let frame_ptr = (user_sp - 8) as *const SignalFrame;
    
    if !crate::syscall::fs::validate_user_ptr(frame_ptr as *const u8, core::mem::size_of::<SignalFrame>()) {
        return Errno::EFAULT.into();
    }
