...
def hmp(cmd, qmp_sock=None, qf_file=None):
    if qmp_sock is None:
        qmp_sock = qmp
    if qf_file is None:
        qf_file = qf
    qmp_sock.sendall(json.dumps({"execute": "human-monitor-command", "arguments": {"command-line": cmd}}).encode() + b'\n')
    while True:
        line = qf_file.readline()
        if not line: break
