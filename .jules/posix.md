...
## 2026-03-30 - Enforcement of EBADF in ftruncate for Read-Only File Descriptors

**Learning:** POSIX.1-2017 and Linux `ftruncate(2)` specifications require `ftruncate` on file descriptors not open for writing to return `-EBADF` (`-9`). Previously, `sys_ftruncate` returned `-EINVAL` (`-22`) when `!flags.is_writable()`.
**Action:** Always check `!flags.is_writable()` in `sys_ftruncate` and return `Errno::EBADF` (`-EBADF`).
