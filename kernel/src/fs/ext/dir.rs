...
impl ExtInode {
    ...
    pub fn add_directory_entry(
        ...
        // Fallback: search earlier blocks from end to start if last block had no space
        for file_block in (0..total_blocks.saturating_sub(1)).rev() {
    ...
    pub fn create_dir_entry(&self, name: &str, file_type: FileType) -> Option<Arc<dyn InodeOps>> {
        ...
        let mut raw_child = ExtRawInode::zeroed();
        raw_child.i_mode = match file_type {
            FileType::Directory => 0x4000 | 0o755,
            FileType::Symlink => 0xA000 | 0o777,
            _ => 0x8000 | 0o644,
        };
        raw_child.i_atime = now;
        raw_child.i_ctime = now;
        raw_child.i_mtime = now;
        raw_child.i_links_count = if is_dir { 2 } else { 1 };
