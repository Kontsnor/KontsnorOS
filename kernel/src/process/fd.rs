...
pub fn current_task_alloc_fd_with_flags_and_path(
    ...
    let max_fd = task.rlimit_nofile_cur as usize;
    let start = fd_table.next_free_fd;

    // Fast-path scan starting from next_free_fd hint
...
pub fn current_task_dup2_fd(oldfd: i32, newfd: i32) -> Option<i32> {
    ...
    *file_desc.ref_count.lock() += 1;
    ...
    if newfd_idx == fd_table.next_free_fd {
        while fd_table.next_free_fd < fd_table.entries.len()
            && fd_table.entries[fd_table.next_free_fd].is_some()
        {
